...
%>
    ...
    <% while(rs.next()){
        ...
        <p class='pay-item-order'> <%= rs.getString("expiration_date") %> &nbsp; &nbsp;&nbsp;<br/><br/> <u>
         <% out.print("<a href='../api/delete_voucher.jsp?id="+id+"' class='button'>");
         out.println("Delete</a></u></p>");
    %>
    <% } %>
    
    <br/>
    <br/>
    <form action='../api/add_voucher.jsp'>
        <input type='text' style="margin-left: 18%" name="discount"/>
        <input type='text' style="margin-left: 9%" name="threshold"/>
        <input type='date' style="margin-left: 8%" name="date"/>
        <br />
        <br />
        <center><input type="submit" value="Add Voucher" style="margin-left: 70%"/></center>
    </form>
<%
// close connection
con.close();
